...
- **Free OpenRouter models for narrative; Claude (this agent) for reasoning + code.** Long-form analyst writeups route through OpenRouter using the model chain in `config.OPENROUTER_MODELS` (default: free JSON-capable models — Nemotron-3-Super-120B → GPT-OSS-20B, tried in order, first valid JSON wins; both confirmed working on the free tier). Override with the `OPENROUTER_MODELS` env var (e.g. set to `anthropic/claude-haiku-4.5` to go back to the paid model). Still needs a free `OPENROUTER_API_KEY`. Fail loudly if routing breaks — never fabricate a narrative. See `ticker-deepdive` skill.
